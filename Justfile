...
build:
    DOCKER_BUILDKIT=1 docker buildx bake -f docker-compose.dev.yml
...
rebuild-frontend:
    DOCKER_BUILDKIT=1 docker compose -f docker-compose.dev.yml up -d --build frontend

# Rebuild and restart backend with fresh dependencies
rebuild-backend:
    DOCKER_BUILDKIT=1 docker compose -f docker-compose.dev.yml up -d --build backend
