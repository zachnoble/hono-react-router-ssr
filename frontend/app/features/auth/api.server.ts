...
export const initiateLogin = async (
	...
	const res = await api.post('/auth/email-otp/send-verification-otp', {
	...
	return redirect(`/login?email=${encodeURIComponent(submission.value.email)}`)
}
...
export const confirmLogin = async (
	...
	const result = await api.post('/auth/sign-in/email-otp', {
		email: submission.value.email,
