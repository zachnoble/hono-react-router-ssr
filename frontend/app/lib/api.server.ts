...
export const createAPIClient = (baseUrl: string, request?: Request): Client => {
	...
	const withHeaders = (options?: RequestOptions) => ({
		...options,
	...
	return {
		get: <T>(path: string, options?: RequestOptions) =>
			fetcher<T>(baseUrl, path, 'GET', withHeaders(options)),
		post: <T>(path: string, body?: unknown, options?: RequestOptions) =>
			fetcher<T>(baseUrl, path, 'POST', { ...withHeaders(options), body }),
		put: <T>(path: string, body?: unknown, options?: RequestOptions) =>
			fetcher<T>(baseUrl, path, 'PUT', { ...withHeaders(options), body }),
		patch: <T>(path: string, body?: unknown, options?: RequestOptions) =>
			fetcher<T>(baseUrl, path, 'PATCH', { ...withHeaders(options), body }),
		delete: <T>(path: string, options?: RequestOptions) =>
			fetcher<T>(baseUrl, path, 'DELETE', withHeaders(options)),
	}
}
