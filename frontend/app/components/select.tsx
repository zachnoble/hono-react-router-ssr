...
export const SelectRoot = <T extends object>(props: RACSelectProps<T>) => {
	return (
		<RACSelect
			{...props}
			data-ui='select'
		...
		<Button
			...
			className={composeRenderProps(
				...
				(className, { isFocusVisible, isPressed, isHovered, isDisabled }) => {
					return twMerge(
						'group relative flex w-full cursor-pointer items-center gap-x-2 rounded-md border border-input text-start shadow-xs outline-hidden transition',
						'ps-3.5 pe-2.5',
		...
		>
			...
			<SelectValue
				...
				className={twMerge([
					...
					'[&>[data-ui=content]_[data-ui=icon]:not([class*=size-])]:size-4',
					'[&>[data-ui=content]_[role=img]]:size-5',
				])}
			/>
			...
			{props.onClear && (
				<span
					...
					className='text-muted hover:text-foreground flex cursor-pointer items-center'
					onPointerDown={(e) => {
						e.stopPropagation()
						props.onClear!()
					}}
			...
			<ChevronDownIcon className='group-not-[&:hover]:text-muted/75 size-4' />
		</Button>
	)
}
...
export interface SelectListBoxProps<T>
	extends ListBoxProps<T>, React.RefAttributes<HTMLDivElement> {
...
export const SelectSection = <T extends object>(props: SectionProps<T>) => {
	return (
		<RACListBoxSection
			className={twMerge(
				'not-first:mt-1.5',
...
export const SelectListItem = React.forwardRef(
	({ destructive, checkIcon, ...props }: SelectListItemProps, ref: React.Ref<HTMLDivElement>) => {
		const textValue =
			...
			<RACListBoxItem
				...
				className={composeRenderProps(
					...
					(className, { isFocused, isDisabled, isHovered }) =>
						twMerge(
							'group flex cursor-pointer select-none items-center gap-x-1.5 rounded-sm outline-hidden',
							'px-2 py-1.5 text-sm/6',
							'[&_[data-ui=icon]:not([class*=size-])]:size-4',
							isDisabled && 'opacity-50',
							(isFocused || isHovered) && 'bg-subtle',
							destructive
								? 'text-red-600'
					...
					return (
						<>
							{checkIcon !== null && (
								<span
									className={twMerge(
										'flex h-lh items-center self-start',
...
export const SelectListItemLabel = ({
	className,
	...props
...
export const Select = ({
	...
	const renderGroups = (groups: OptionGroup[]) => {
		return groups.map((group) => (
			<SelectSection key={group.title} title={group.title}>
	...
	const isGrouped =
		Array.isArray(options) && options.length > 0 && options[0] && 'options' in options[0]
	...
	return (
		...
		>
			<SelectButton onClear={clearable && value ? () => onChange('') : undefined} />
			<SelectPopover>
				<SelectListBox>
					{isGrouped
