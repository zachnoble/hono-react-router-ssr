...
const getInitialsGradient = (name: string, colorful?: boolean): [string, string] => {
	if (colorful) {
		const i = sumChars(name) % GRADIENTS.length
		return GRADIENTS[i] ?? GRADIENTS[0] ?? ['#000000', '#000000']
	}

	return GRADIENTS[0] ?? ['#000000', '#000000']
}

export const getFallbackAvatarDataUrl = ({
	alt,
	fallback,
	colorful,
	background,
...
const getFallbackIconDateUrl = (bg: string) => {
...
const getFallbackInitialsDataUrl = (bg: string, initials: string) => {
	return `data:image/svg+xml;base64,${btoa(
