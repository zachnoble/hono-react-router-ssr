	...
	const style = {
		...
		variant: {
			...
			solid: [
				...
				'[--icon-color:white]',
				'text-white',
				'ring-1 ring-inset ring-white/15 dark:ring-[oklch(from_var(--btn-bg)_calc(l*1.15)_c_h)]',
			...
			outline: [
				...
				!isDisabled && 'hover:bg-subtle! dark:hover:bg-zinc-800!',
			],
			plain: ['text-[var(--btn-color)]', !isDisabled && 'hover:bg-subtle'],
			link: [
				'text-muted font-normal underline [&:not(:hover)]:decoration-[var(--btn-color)]/15 underline-offset-4',
			],
		},
		size: {
			base: 'text-sm/6 [&_svg[data-ui=icon]:not([class*=size-])]:size-[var(--icon-size)]',
			sm: [
				isIconOnly
					? 'size-7 [--icon-size:theme(size.4)]'
					: variant !== 'link' && 'h-7 [--icon-size:theme(size.3)] text-xs/6 px-2',
			],
			md: [
				'[--icon-size:theme(size.4)]',
				isIconOnly
					? 'p-1.5 [&_svg[data-ui=icon]]:m-1'
					: variant !== 'link' && 'px-3 py-1.5',
			],

			lg: [
				'[--icon-size:theme(size.5)]',
				isIconOnly
					? 'p-2 [&_svg[data-ui=icon]]:m-0.5'
					: variant !== 'link' && 'px-3.5 py-2',
			],
		},
		color: {
