...
export const TagGroup = (props: AriaTagGroupProps) => {
...
export const Tag = ({ children, color = 'zinc', ...props }: TagProps) => {
		...
		<AriaTag
			...
			className={composeRenderProps(
				...
				(className, { isFocusVisible, isDisabled }) => {
					return twMerge(
						'flex max-w-fit cursor-pointer items-center gap-x-1 rounded-md px-2 py-0.5 font-medium text-xs/5 outline-0 transition',
							...
							<Button
								...
								className={composeRenderProps(
									...
									(className, { isPressed, isHovered, isFocusVisible }) =>
										twMerge(
											'flex cursor-pointer items-center justify-center rounded-full p-0.5 outline-0 transition-[background-color]',
