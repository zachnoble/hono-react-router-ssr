...
export const Table = (props: TableProps) => {
	return (
		<ResizableTableContainer className='relative max-h-70 w-137.5 scroll-pt-[2.281rem] overflow-auto rounded-md border'>
	...
	return (
		<AriaColumn
			...
			className={composeTailwindRenderProps(
				props.className,
				'cursor-pointer border-b text-start font-semibold text-sm focus-within:z-20 [&:hover]:z-20',
			)}
		>
				...
				<div className='flex items-center'>
					...
					>
						...
						{allowsSorting && (
							...
							>
								{sortDirection && <ChevronUpIcon className='text-muted size-4' />}
							</span>
						)}
					</Group>
					{!props.width && (
						<ColumnResizer
							className={composeRenderProps(
								...
								(className, { isFocusVisible, isResizing }) =>
									twMerge(
										'box-content h-5 w-[1.5px] translate-x-2 cursor-col-resize rounded-sm bg-border bg-clip-content px-2 py-1',
										isResizing &&
...
export const Row = <T extends object>({ id, columns, children, ...props }: RowProps<T>) => {
		...
		<AriaRow
			...
			className={composeRenderProps(
				...
				(className, { isFocusVisible, isSelected, isHovered, isDisabled }) =>
					twMerge(
						'group/row relative cursor-pointer select-none text-sm',
