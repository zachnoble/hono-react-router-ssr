		...
		<AvatarContext.Provider value={{ badgeId }}>
			<div
				...
				className={twMerge([
					...
					status === 'loaded' &&
						'dark:outline dark:outline-white/10 dark:-outline-offset-1',
...
export const AvatarBadge = ({ badge, ...props }: AvatarBadgeProps) => {
	...
	return (
		<span
			...
			id={context.badgeId}
			className={twMerge([
				'absolute inset-e-0 bottom-0 grid place-items-center rounded-full bg-background bg-clip-content',
				props.className,
			])}
		>
			{badge}
...
export const AvatarGroup = ({ reverse = false, className, ...props }: AvatarGroupProps) => {
	return (
		<div
			{...props}
			className={twMerge(
