...
export const RadioGroup = (props: RACRadioGroupProps) => {
	return (
		<RACRadioGroup
			{...props}
...
export const Radios = ({ className, ...props }: React.JSX.IntrinsicElements['div']) => {
	return (
		<div
			data-ui='box'
			className={twMerge(
...
export const RadioField = ({ className, ...props }: React.JSX.IntrinsicElements['div']) => {
	return (
		<DescriptionProvider>
			<div
				{...props}
...
export interface CustomRenderRadioProps extends Omit<RACRadioProps, 'children' | 'render'> {
	render: string | React.ReactElement | ((props: RadioRenderProps) => React.ReactNode)
	radio?: never
	children?: never
}

export const Radio = (props: RadioProps | CustomRenderRadioProps) => {
	...
	return (
		<RACRadio
			...
			className={composeRenderProps(props.className, (className, { isDisabled }) =>
				twMerge(
					'group flex cursor-pointer items-center text-sm/6',
							...
							{radio ? (
								...
								<div
									className={twMerge(
										'rounded-full',
										renderProps.isSelected &&
											'size-1.5 bg-[lch(from_var(--color-accent)_calc((49.44-l)*infinity)_0_0)]',
									)}
								/>
							)}
						</div>
