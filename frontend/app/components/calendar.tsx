							...
							<CalendarCell
								...
								className={composeRenderProps(
									...
									(
										...
										return twMerge(
											'relative flex size-10 cursor-pointer items-center justify-center rounded-lg text-sm outline-hidden',
											...
											isSelected && [
												'bg-accent text-[lch(from_var(--color-accent)_calc((49.44-l)*infinity)_0_0)] text-sm',
...
export const CalendarHeader = ({ yearRange }: { yearRange?: YearRange }) => {
	...
	return (
		<header className={twMerge('flex w-full items-center pe-2', yearRange ? 'ps-2' : 'ps-3')}>
			{yearRange ? (
				<div className='flex flex-1 gap-x-2 text-left text-sm/6 rtl:text-right'>
				...
				<Heading
					level={2}
					className='flex flex-1 text-left text-sm/6 font-medium rtl:text-right'
					aria-hidden
				/>
			)}
...
const YearDropdown = ({ state, yearRange }: { state: CalendarState; yearRange: YearRange }) => {
	const years: Array<{
		value: CalendarDate
		formatted: string
	}> = []
...
const MonthDropdown = ({ state }: { state: CalendarState }) => {
