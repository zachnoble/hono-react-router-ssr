...
export const Menu = <T extends object>({ checkIconPlacement = 'end', ...props }: MenuProps<T>) => {
	return (
		<RACMenu
			{...props}
...
export const SubMenu = <T extends object>(props: MenuProps<T> & { 'aria-label': string }) => {
	return <Menu {...props} />
}
...
export const MenuItem = ({ destructive, ...props }: MenuItemProps) => {
	const textValue =
		...
		<RACMenuItem
			...
			className={composeRenderProps(
				...
				(className, { isFocused, isDisabled }) => {
					return twMerge([
						'group cursor-pointer rounded-sm outline-hidden',
...
export const MenuItemLabel = ({ className, ...props }: React.JSX.IntrinsicElements['span']) => {
	return (
...
export const MenuSection = <T extends object>({ className, ...props }: MenuSectionProps<T>) => {
	return (
		...
		>
			<Header className='bg-background text-muted sticky inset-0 z-10 truncate pt-2 text-xs/6 rtl:text-right'>
				{props.title}
			</Header>
